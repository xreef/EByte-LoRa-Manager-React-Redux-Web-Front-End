...
import {
    DEVICE_MESSAGES_FIELD_UPDATED, DEVICE_MESSAGES_SEND_TRANSPARENT, MESSAGE_SIZE, //key, DeviceMessagesActions
} from "../types/deviceMessages";
import {
    deviceMessagesFieldInvalid,
    deviceMessagesSendTransparentSuccess, deviceMessagesSendTransparentFailed, addNotification, /*deviceMessagesFieldUpdated*/
} from "../actions";
// import {
//   addNotification
// } from '../actions/notifications';
import { deviceMessagesSelectors } from '../reducers/deviceMessages';
import { TRANSPARENT_TRANSMISSION_ENDPOINT, MICROCONTROLLER_ADRESS } from '../config';
 ...
 */
export function validateFields(deviceMessage: string) {
  const errors = [];
 ...
 * Dispatch DEVICE_MESSAGES_SEND_TRANSPARENT_SUCCESS or DEVICE_MESSAGES_SEND_TRANSPARENT_FAILED
 * based on the response from the server.
 * Note: axios was injected as httpClient in
 * src/configureStore.js
 */
export const deviceMessagesSendTransparentLogic = createLogic({
  type: DEVICE_MESSAGES_SEND_TRANSPARENT,
  validate({ httpClient, action }, allow, reject) {
    // const state: any = getState();
    // const dataToUpdate = state.deviceMessages.deviceMessages; //deviceMessagesSel.deviceMessages(state);
      const dataToUpdate = action.deviceMessage;
    const errors = validateFields(dataToUpdate);
    if (!errors.length) {
      allow(action); // no errors, let DEVICE_MESSAGES_SEND_TRANSPARENT go through
  ...
  process({ httpClient, getState, action}, dispatch, done) {
      const dataToUpdate = {"message": action.deviceMessage};
    httpClient.post(`http://${MICROCONTROLLER_ADRESS}/${TRANSPARENT_TRANSMISSION_ENDPOINT}`, dataToUpdate)
      .then(resp => resp.data) // new user created is returned
      .then((respData) => {
          dispatch(addNotification({ message: <FormattedMessage  id="deviceMessages.transparent.send.success" values={{creturn: <br/>, message: <span><i><br/>{action.deviceMessage}</i></span>}} />, variant: 'success' }));

          dispatch(deviceMessagesSendTransparentSuccess(action.deviceMessage));
      })
      .catch(err => {
        console.error(err); // might be a render err
        dispatch(deviceMessagesSendTransparentFailed(err));
...
export default [
  deviceMessagesSendTransparentLogic
];
