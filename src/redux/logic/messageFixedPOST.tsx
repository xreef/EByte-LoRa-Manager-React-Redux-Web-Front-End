...
import {
    DEVICE_MESSAGES_FIELD_UPDATED, DEVICE_MESSAGES_SEND_FIXED, MESSAGE_SIZE, //key, DeviceMessagesActions
} from "../types/deviceMessages";
import {
    deviceMessagesFieldInvalid,
    deviceMessagesSendFixedSuccess, deviceMessagesSendFixedFailed, addNotification, /*deviceMessagesFieldUpdated*/
} from "../actions";
// import {
//   addNotification
...
import { FIXED_TRANSMISSION_ENDPOINT, MICROCONTROLLER_ADRESS } from '../config';
 ...
 */
export function validateFields(CHAN: number | undefined, ADDH: number | undefined, ADDL: number | undefined, deviceMessage: string, configuration: IConfiguration | undefined) {
  const errors = [];
  ...
  // if (data.serverSMTP && (!data.serverSMTP.from || !data.serverSMTP.password || !data.serverSMTP.port || !data.serverSMTP.server || !data.serverSMTP.login)) { errors.push('SMTP deviceMessages not correct'); }
    ...
    if (configuration && ADDH==configuration.ADDH && ADDL==configuration.ADDL && CHAN==configuration.CHAN)
      errors.push('You can\'t send a message to the same device, change address or channel');
  return errors;
}
 ...
 * Dispatch DEVICE_MESSAGES_SEND_FIXED_SUCCESS or DEVICE_MESSAGES_SEND_FIXED_FAILED
 * based on the response from the server.
 * Note: axios was injected as httpClient in
 * src/configureStore.js
 */
export const deviceMessagesSendFixedLogic = createLogic({
  ...
  validate({ httpClient, action, getState }, allow, reject) {
    ...
    if (!errors.length) {
      allow(action); // no errors, let DEVICE_MESSAGES_SEND_FIXED go through
  ...
  process({ httpClient, getState, action}, dispatch, done) {
      const dataToUpdate = {CHAN: action.CHAN, ADDL: action.ADDL, ADDH: action.ADDH, "message": action.deviceMessage};
    httpClient.post(`http://${MICROCONTROLLER_ADRESS}/${FIXED_TRANSMISSION_ENDPOINT}`, dataToUpdate)
      .then(resp => resp.data) // new user created is returned
      .then((respData) => {
          dispatch(addNotification({ message: <FormattedMessage  id="deviceMessages.fixed.send.success"
                                                                 values={
                                                         {
                                                             creturn: <br/>,
                                                            CHAN: action.CHAN,
          ...
          dispatch(deviceMessagesSendFixedSuccess(action.CHAN, action.ADDH, action.ADDL, action.deviceMessage));
      })
      .catch(err => {
        console.error(err); // might be a render err
        dispatch(deviceMessagesSendFixedFailed(err));
